...
class Engineer_details_model extends CI_Model {       
	...
	/**
      ...
      * @param : $id is value of engineer_details_id
      */
	public function Get_data_id($id='') {
		 $this->db->select('*');
		 $this->db->from('engineer_details');
		 $this->db->where('engineer_details_id' , $id);
		 $query = $this->db->get();
	...
	public function get_data($con=NULL) {
		if(CheckPermission('engineer_details', "own_read") && CheckPermission('engineer_details', "all_read")!=true){
			if($con != '') {
				$con .= " AND "; 
			}
			$con .= "  (`engineer_details`.`user_id` = '".$this->user_id."') ";
		}
		$sql = "SELECT * FROM  `engineer_details` ";
		if($con != '') {
			$sql .= ' WHERE '.$con;	
		}
