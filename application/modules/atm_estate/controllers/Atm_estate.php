...
class Atm_estate extends CI_Controller {
  	function __construct() {
	    parent::__construct();
	    $this->load->model("Atm_estate_model"); 
	    $this->lang->load(strtolower('Atm_estate'), setting_all('language'));
		if(true==1){
			is_login(); 
      ...
      */
  	public function index() {   
  		if(CheckPermission("atm_estate", "all_read,own_read")){
  			$con = '';
  			
			$data["view_data"]= $this->Atm_estate_model->get_data($con);
			if($con == '') {
  	...
  	public function get_filter_html($ajax = 0) {
  		$filter_coo = '';
  		if(isset($_COOKIE[strtolower('Atm_estate_filter')])) {
  			$filter_coo = json_decode($_COOKIE[strtolower('Atm_estate_filter')]);
  		}
  		$html = '';
  		if($ajax == 1) {
  			echo $html; exit;
  	...
  	public function get_filter_dropdown_options($field, $rel_table, $rel_col, $selected) {
  		$res = $this->Atm_estate_model->get_filter_dropdown_options($field, $rel_table, $rel_col);
  		$option = '';
	...
	public function add_edit() {	
		...
		if($this->input->post('id')) {
			...
			if(isset($data['mkacf'])) {
                ...
                if(!empty($custom_fields)) {
                    foreach ($custom_fields as $cfkey => $cfvalue) {
                        ...
                        $cf_data = $this->Atm_estate_model->getQrResult($qr);
                        if(is_array($cf_data) && !empty($cf_data)) {
                            $d = array(
                                        "value" => $custom_fields[$cf_data[0]->cf_id],
                                    );
                            $this->Atm_estate_model->updateRow('cf_values', 'cf_values_id', $cf_data[0]->cf_values_id, $d);
                        } else {
                            $d = array(
                            ...
                            $this->Atm_estate_model->insertRow('cf_values', $d);
                        }
                    }
                }
            }
			...
			$this->Atm_estate_model->updateRow('atm_estate', 'atm_estate_id', $this->input->post('id'), $data);
			...
			$last_id = $this->Atm_estate_model->insertRow('atm_estate', $data);
			if(!empty($custom_fields)) {
                foreach ($custom_fields as $cfkey => $cfvalue) {
                	if(is_array($cfvalue)) {
                		$cfvalue = implode(',', $cfvalue);
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Atm_estate_model->Get_data_id($this->input->post('id'));
	...
	/**
      ...
      */
  	public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Atm_estate_model->delete_data($value);		
		}
		echo json_encode($idsArr); 
		exit;
		//redirect(base_url().'atm_estate', 'refresh');
  	}
  	/**
      ...
      */
  	public function delete_data($id) { 
		$this->Atm_estate_model->delete_data($id);
	...
	/**
      ...
      */
  	public function ajx_data(){
		$primaryKey = 'atm_estate_id';
		$table 		= 'atm_estate';
		$joinQuery  =   "FROM `atm_estate` ";
		$columns 	= array(
array( 'db' => 'atm_estate.atm_estate_id AS atm_estate_id', 'field' => 'atm_estate_id', 'dt' => 0 ),
array( 'db' => 'atm_estate.atm_estate_bank_name AS atm_estate_bank_name', 'field' => 'atm_estate_bank_name', 'dt' => 1 ),
array( 'db' => 'atm_estate.atm_estate_quantity AS atm_estate_quantity', 'field' => 'atm_estate_quantity', 'dt' => 2 ),
);

			error_reporting(E_ALL);
			ini_set('display_errors', 1);
	        $cf = get_cf('atm_estate');
	        if(is_array($cf) && !empty($cf)) {
	            foreach ($cf as $cfkey => $cfvalue) {
	                ...
	                $joinQuery  .=  " LEFT JOIN `cf_values` AS cf_values_".$cfkey."  ON  `atm_estate`.`atm_estate_id` = `cf_values_".$cfkey."`.`rel_crud_id` AND `cf_values_".$cfkey."`.`cf_id` =  '".$cfvalue->custom_fields_id."' ";
	            }
	        }
			array_push($columns, array( 'db' => 'atm_estate.atm_estate_id AS atm_estate_id', 'field' => 'atm_estate_id', 'dt' => count($columns) ));
		$where = '';
		$j = 0;
      ...
      */
  	...
  	public function set_filter_cookie() {
  		if(!empty($this->input->post(strtolower('Atm_estate_filter')))) {
  			setcookie(strtolower('Atm_estate_filter'), json_encode($this->input->post(strtolower('Atm_estate_filter'))), time() + (86400 * 30 * 365), "/");
  		}
  	}
		...
		$result = array();
  		if(is_array($this->input->get('request')) && !empty($this->input->get('request'))) {
  			foreach ($this->input->get('request') as $rkey => $rvalue) {
				...
				$res = $this->Atm_estate_model->getQrResult($qr);
