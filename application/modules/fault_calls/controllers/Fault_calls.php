...
class Fault_calls extends CI_Controller {
  	function __construct() {
	    parent::__construct();
	    $this->load->model("Fault_calls_model"); 
	    $this->lang->load(strtolower('Fault_calls'), setting_all('language'));
		if(true==1){
			is_login(); 
      ...
      */
  	public function index() {   
  		if(CheckPermission("fault_calls", "all_read,own_read")){
  			$con = '';
  			
			$data["view_data"]= $this->Fault_calls_model->get_data($con);
			if($con == '') {
  	...
  	public function get_filter_html($ajax = 0) {
  		$filter_coo = '';
  		if(isset($_COOKIE[strtolower('Fault_calls_filter')])) {
  			$filter_coo = json_decode($_COOKIE[strtolower('Fault_calls_filter')]);
  		}
  		$html = '';
  		if($ajax == 1) {
  			echo $html; exit;
  	...
  	public function get_filter_dropdown_options($field, $rel_table, $rel_col, $selected) {
  		$res = $this->Fault_calls_model->get_filter_dropdown_options($field, $rel_table, $rel_col);
  		$option = '';
	...
	public function add_edit() {	
		...
		if($this->input->post('id')) {
			...
			if(isset($data['mkacf'])) {
                ...
                if(!empty($custom_fields)) {
                    foreach ($custom_fields as $cfkey => $cfvalue) {
                        ...
                        $cf_data = $this->Fault_calls_model->getQrResult($qr);
                        if(is_array($cf_data) && !empty($cf_data)) {
                            $d = array(
                                        "value" => $custom_fields[$cf_data[0]->cf_id],
                                    );
                            $this->Fault_calls_model->updateRow('cf_values', 'cf_values_id', $cf_data[0]->cf_values_id, $d);
                        } else {
                            $d = array(
                            ...
                            $this->Fault_calls_model->insertRow('cf_values', $d);
                        }
                    }
                }
            }
			...
			$this->Fault_calls_model->updateRow('fault_calls', 'fault_calls_id', $this->input->post('id'), $data);
			...
			$last_id = $this->Fault_calls_model->insertRow('fault_calls', $data);
			if(!empty($custom_fields)) {
                foreach ($custom_fields as $cfkey => $cfvalue) {
                	if(is_array($cfvalue)) {
                		$cfvalue = implode(',', $cfvalue);
	...
	public function get_modal() {
		if($this->input->post('id')){
			$data['data']= $this->Fault_calls_model->Get_data_id($this->input->post('id'));
	...
	/**
      ...
      */
  	public function delete($ids) {
		...
		foreach ($idsArr as $key => $value) {
			$this->Fault_calls_model->delete_data($value);		
		}
		echo json_encode($idsArr); 
		exit;
		//redirect(base_url().'fault_calls', 'refresh');
  	}
  	/**
      ...
      */
  	public function delete_data($id) { 
		$this->Fault_calls_model->delete_data($id);
	...
	/**
      ...
      */
  	public function ajx_data(){
		$primaryKey = 'fault_calls_id';
		$table 		= 'fault_calls';
		$joinQuery  =   "FROM `fault_calls` ";
		$columns 	= array(
array( 'db' => 'fault_calls.fault_calls_id AS fault_calls_id', 'field' => 'fault_calls_id', 'dt' => 0 ),
array( 'db' => 'fault_calls.fault_calls_engineer_name AS fault_calls_engineer_name', 'field' => 'fault_calls_engineer_name', 'dt' => 1 ),
array( 'db' => 'fault_calls.fault_calls_bank_name AS fault_calls_bank_name', 'field' => 'fault_calls_bank_name', 'dt' => 2 ),
array( 'db' => 'fault_calls.fault_calls_terminal_id AS fault_calls_terminal_id', 'field' => 'fault_calls_terminal_id', 'dt' => 3 ),
array( 'db' => 'fault_calls.fault_calls_terminal_name AS fault_calls_terminal_name', 'field' => 'fault_calls_terminal_name', 'dt' => 4 ),
array( 'db' => 'fault_calls.fault_calls_terminal_location AS fault_calls_terminal_location', 'field' => 'fault_calls_terminal_location', 'dt' => 5 ),
array( 'db' => 'fault_calls.fault_calls_fault_description AS fault_calls_fault_description', 'field' => 'fault_calls_fault_description', 'dt' => 6 ),
array( 'db' => 'fault_calls.fault_calls_error_code AS fault_calls_error_code', 'field' => 'fault_calls_error_code', 'dt' => 7 ),
array( 'db' => 'fault_calls.fault_calls_date_logged AS fault_calls_date_logged', 'field' => 'fault_calls_date_logged', 'dt' => 8 ),
array( 'db' => 'fault_calls.fault_calls_time_loggged AS fault_calls_time_loggged', 'field' => 'fault_calls_time_loggged', 'dt' => 9 ),
array( 'db' => 'fault_calls.fault_calls_status AS fault_calls_status', 'field' => 'fault_calls_status', 'dt' => 10 ),
array( 'db' => 'fault_calls.fault_calls_date_resolved AS fault_calls_date_resolved', 'field' => 'fault_calls_date_resolved', 'dt' => 11 ),
array( 'db' => 'fault_calls.fault_calls_time_resolved AS fault_calls_time_resolved', 'field' => 'fault_calls_time_resolved', 'dt' => 12 ),
);

			error_reporting(E_ALL);
			ini_set('display_errors', 1);
	        $cf = get_cf('fault_calls');
	        if(is_array($cf) && !empty($cf)) {
	            foreach ($cf as $cfkey => $cfvalue) {
	                ...
	                $joinQuery  .=  " LEFT JOIN `cf_values` AS cf_values_".$cfkey."  ON  `fault_calls`.`fault_calls_id` = `cf_values_".$cfkey."`.`rel_crud_id` AND `cf_values_".$cfkey."`.`cf_id` =  '".$cfvalue->custom_fields_id."' ";
	            }
	        }
			array_push($columns, array( 'db' => 'fault_calls.fault_calls_id AS fault_calls_id', 'field' => 'fault_calls_id', 'dt' => count($columns) ));
		$where = '';
		$j = 0;
      ...
      */
  	public function getFilterdata(){
		...
		if($this->input->post('dateRange')) {
			...
			$where = " DATE_FORMAT(`fault_calls`.`".$this->input->post('colName')."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND  DATE_FORMAT(`fault_calls`.`".$this->input->post('colName')."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
		}
		$data["view_data"]= $this->Fault_calls_model->get_data($where);
		echo $this->load->view("tableData",$data, true);
		die;
  	}

  	public function set_filter_cookie() {
  		if(!empty($this->input->post(strtolower('Fault_calls_filter')))) {
  			setcookie(strtolower('Fault_calls_filter'), json_encode($this->input->post(strtolower('Fault_calls_filter'))), time() + (86400 * 30 * 365), "/");
  		}
  	}
		...
		$result = array();
  		if(is_array($this->input->get('request')) && !empty($this->input->get('request'))) {
  			foreach ($this->input->get('request') as $rkey => $rvalue) {
				...
				$res = $this->Fault_calls_model->getQrResult($qr);
