...
class SSP {
    ...
    static function column_mka_filter ($request, $where = '') {
        foreach ($request as $gkey => $gvalue) {
             if (strpos($gkey, '___filter') !== false && $gvalue != 'all') {
                $fa = explode('___', $gkey);
                $and = '';
                if($where !== '') {
                    $and = ' AND ';
                ...
                if($fa[2] === 'filter_date') {
                    $date = explode(' - ', $gvalue);
                    $where .= $and." DATE_FORMAT(`".$fa[0]."`.`".$fa[1]."`, '%Y/%m/%d') >= '".date('Y/m/d', strtotime($date[0]))."' AND  DATE_FORMAT(`".$fa[0]."`.`".$fa[1]."`, '%Y/%m/%d') <= '".date('Y/m/d', strtotime($date[1]))."' ";
                } else {
                    $fl = '`'.$fa[0].'`.`'.$fa[1].'`';
                    $where .= $and ."".$fl." LIKE  '%".$gvalue."%'";
                }
            }
        }
